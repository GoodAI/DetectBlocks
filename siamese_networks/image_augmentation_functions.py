...
def crop_and_reshape(in_tensor, zoom_value):
    ...
    crop_offset_height = tf.cast(tf.math.floordiv(tf.subtract(in_tensor_shape[1], new_size1), 2), tf.int32)
    crop_offset_width = tf.cast(tf.math.floordiv(tf.subtract(in_tensor_shape[2], new_size2), 2), tf.int32)
    ...
    img = tf.image.resize(img, [in_tensor_shape[1], in_tensor_shape[2]], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def reshape_and_pad(in_tensor, zoom_value, mode):
    ...
    #resize the image
    img = tf.image.resize(in_tensor, new_size, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    padding = [[0, 0],
        [tf.cast(tf.math.floordiv(pad1, 2), dtype=tf.int32) + tf.math.floormod(pad1, 2), tf.cast(tf.math.floordiv(pad1, 2), dtype=tf.int32)],
        [tf.cast(tf.math.floordiv(pad2, 2), dtype=tf.int32) + tf.math.floormod(pad2, 2), tf.cast(tf.math.floordiv(pad2, 2), dtype=tf.int32)], 
        [0, 0]]
    #pad the image
...
def zoom_image_object(in_tensor, zoom_range, padding_mode = 'CONSTANT'):
    ...
    zoom_value = tf.random.shuffle(zoom_range)[0]
...
def translate_im_obj(in_tensor, trans_range):
    translation = tf.to_float(tf.random.uniform([in_tensor.get_shape().as_list()[0], 2], 
                        trans_range[0],
                        trans_range[1],
                        dtype = tf.int32))
...
def in_plane_rotation(in_tensor):
    rad_rot = tf.random.uniform([], 0, 2 * pi)
...
def add_black_patches(in_tensor, max_area_cov = 0.25, max_patch_nb = 5):
    ...
    def square_patch(in_tensor, max_coverage = 0.05):
        ...
        coverage = tf.random.uniform([], minval = 0.01, maxval = max_coverage)
        ...
        # #print(shape)
        # brightness_offset = tf.random.uniform([shape[0]], minval = -max_offset, maxval = max_offset, dtype = tf.float32)
        
        x_offset = tf.random.uniform([in_tensor_shape_list[0], 1], 0, max_x, dtype = tf.int32)
        y_offset = tf.random.uniform([in_tensor_shape_list[0], 1], 0, max_y, dtype = tf.int32)
    ...
    for i in range(patches):
        prob = tf.random.uniform([], minval = 0.0, maxval = 1.0, dtype = tf.float32)
...
def invert_color(in_tensor):
    ...
    def _one_channel(in_tensor):
        channel = tf.random.uniform([], minval = 0, maxval = 3, dtype = tf.int32)
...
def gaussian_blur(in_tensor, size = 5):
    ...
    def _blur(in_tensor, size=5):
        std = tf.random.uniform([], 0.0, 1.0, dtype = tf.float32)
...
def contrast_normalization(in_tensor, f = [0.5, 2.0]):
    ...
    def _normalize(in_tensor, f):
        ...
        shape = in_tensor.get_shape().as_list()
        factor = tf.tile(tf.random.uniform([shape[0]], f[0], f[1], dtype = tf.float32)[:, tf.newaxis, tf.newaxis, tf.newaxis],
    ...
    def _one_channel(in_tensor, f):
        ...
        shape = in_tensor.get_shape().as_list()
        factor = tf.tile(tf.random.uniform([shape[0]], f[0], f[1], dtype = tf.float32)[:, tf.newaxis, tf.newaxis],
            [1, shape[1], shape[2]])
        
        c = tf.random.uniform([shape[0]], 0, 3, dtype = tf.int32)
...
def gamma_normalization(in_tensor, f = [0.5, 2.2]):
    ...
    def _normalize(in_tensor, f):
        ...
        shape = in_tensor.get_shape().as_list()
        gamma = tf.random.uniform([shape[0]], f[0], f[1], dtype = tf.float32)
...
def multiply_brightness(in_tensor, factor_range = [0.6, 1.4]):
    ...
    shape = in_tensor.get_shape().as_list()
    factor = tf.random.uniform([shape[0]], minval = factor_range[0], maxval = factor_range[1], dtype = tf.float32)
...
def main():
    #bg_img = tf.random.uniform([100, 64, 64, 1], 0.0, 1.0, dtype=tf.float32)
    ...
    #alpha = tf.random.uniform([in_image.get_shape().as_list()[0]], f[0], f[1], dtype = tf.float32)
